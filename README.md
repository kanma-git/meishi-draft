...
【ローカルPC（Cursor / フォルダ）】
　自分のMacの中にある作業フォルダ
　/Users/yutakanma/my-gas-project/
　　　　　↕ git push / git pull
...
`/Users/yutakanma/my-gas-project/コード.js` を開いて上書き保存します。
...
### `cd: no such file or directory: my-gas-project`
→ すでにフォルダの中にいます。`cd` は不要です。そのまま次のコマンドを実行してください。
